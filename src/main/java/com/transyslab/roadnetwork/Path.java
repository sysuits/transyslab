...
public class Path implements NetworkObject {
	...
	public static int MAX_PATH_LENGTH = 20;
	...
	public Link getNextLink(Link l){
		if (links.contains(l)){
			return getLink(links.indexOf(l)+1);
		}
		else
			return null;
	}
	public Link getPreviousLink(Link l){
		if (links.contains(l)){
			return getLink(links.indexOf(l)-1);
		}
		else
	...
	/*
	public int loadSimulationFiles(int id, int oriId, int desId) {
