...
public class Link extends DefaultWeightedEdge implements NetworkObject{
	...
	public void init(long id, int type, String name, int index,Node upNode, Node dnNode){
		...
		if(upNode == null || dnNode == null)
			System.out.print("Error in loadSimulationFiles link");
	...
	public void calcStaticInfo() {
		...
		for (int j = 0; j < ctrlPoints.size(); j++) {
			ctrlPoints.set(j,getNetwork().worldSpace.worldSpacePoint(ctrlPoints.get(j)));
		}
	...
	public void setCtrlPoints(List<GeoPoint> points){
		getNetwork().worldSpace.recordExtremePoints(points);
		ctrlPoints = points;
	}
