...
public class Lane implements NetworkObject,Comparable<Lane> {
	...
	public boolean allowRightTurn(){
		return (rules & Constants.LANE_CHANGE_RIGHT) != 0;
	}

	public boolean allowLeftTurn(){
		return (rules & Constants.LANE_CHANGE_LEFT) != 0;
	}

	public boolean checkTurn(int turnIdx){
		return (rules & turnIdx) != 0;
	}
	...
	public void init(int id, int r, int index, double beginx, double beginy, double endx, double endy, Segment seg) {
		...
		if (this.segment != null) {
			System.out.print("Can't not loadSimulationFiles segment twice");
			return ;
		}
		else {
			this.segment = seg;
