...
public class OptToolBox {
	...
	public static DifferentialEvolution createAlgorithm(String[] args) {
		...
		int popSize = args.length>1 ? Integer.parseInt(args[1]) : 20;
		int maxGeneration = args.length>2 ? Integer.parseInt(args[2]) : 1000;
		double crossOver_cr = args.length>3 ? Integer.parseInt(args[3]) : 0.5;
		double crossOver_f = args.length>4 ? Integer.parseInt(args[4]) : 0.5;
