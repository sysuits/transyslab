...
//	public int di;//弃用
	public List<MLPLane> successiveDnLanes;
...
//		RtCutinAllowed = true;
	...
	public boolean checkVolum(MLPVehicle mlpv) {
		return checkVolum(mlpv.getLength(), mlpv.getCurrentSpeed());
	}
	
	public boolean checkVolum(double vehLen, double vehSpeed) {
		MLPVehicle tail_ = getTail();
		return tail_ == null ||
				spareDis(vehLen, vehSpeed) > 0;
	}

	public boolean checkNewVolum(MLPVehicle mlpv) {
		MLPVehicle tail_ = getTail();
		return tail_ == null ||
				getLength() - minTransposeDis(mlpv.newSpeed) - Math.max(0,tail_.newDis - tail_.getDistance()) - mlpv.getLength() > 0;
	}

	public double spareDis(double vehLen, double vehSpeed) {
		return getLength() - minTransposeDis(vehSpeed) - vehLen;
	}

	public double minTransposeDis(double vehSpeed) {
		if (getTail()==null)
			return 0.0;
		double minus = rearBumperDis();
		return minus + ((MLPParameter) getNetwork().getSimParameter()).minGap(vehSpeed);
	}

	public double maxTransposeSpd(double plusDis) {
		if (getTail()==null)
			return (plusDis + getLength()) / getNetwork().getSimClock().getStepSize();
		double maxGap = plusDis + getLength() - rearBumperDis();
		return ((MLPParameter)getNetwork().getSimParameter()).maxSpeed(maxGap);
	}

	public double rearBumperDis(){
		MLPVehicle tail = getTail();
		return tail==null ? 0.0 : tail.getDistance() + tail.getLength();
	}

	public double rearBumperDsp(){
		return getSegment().endDSP - rearBumperDis();
	}
	...
	public void passVeh(MLPVehicle theVeh, MLPLane target) {
		removeVeh(theVeh,false);
		target.vehsOnLn.offer(theVeh);
		theVeh.lane = target;
		theVeh.segment = target.getSegment();
		theVeh.link = (MLPLink) target.getLink();
	}
