...
//	public List<MLPLoop> sensors;
	...
	protected AllDirectedPaths allDirectedPaths;
	...
	public Link createLink(long id, int type, String name,long upNodeId, long dnNodeId) {
		MLPLink newLink = Installer.getInstance(MLPLink.class);
	...
	public Segment createSegment(long id, int speedLimit, double freeSpeed, double grd, List<GeoPoint> ctrlPoint) {
		MLPSegment newSegment = Installer.getInstance(MLPSegment.class);
	...
	public Sensor createSensor(String detName, long linkId, double portion, double interval) {
		MLPLink lnk = findLink(linkId);
		double dsp = lnk.length()*portion;
		MLPSegment seg = (MLPSegment) lnk.getSegments().stream()
				.filter(s->((MLPSegment)s).startDSP<dsp && ((MLPSegment)s).endDSP>=dsp).findFirst().orElse(null);
		if (seg!=null){
			for (int i = 0; i < seg.nLanes(); i++) {
				MLPLane ln = seg.getLane(i);
				MLPLoop loop = new MLPLoop(ln, seg, lnk, detName, dsp, portion);
				sensors.add(loop);
			}
		}
		return null;
		...
		links.parallelStream().forEach(l->{
			...
			for (Segment seg: segments) {
				for (int i = 0; i < seg.nLanes(); i++) {
					...
					if (i<seg.nLanes()-1 && mlpLane.allowRightTurn())
						((MLPLink)l).addLaneGraphEdge(mlpLane, (MLPLane) mlpLane.getRightLane(), 1.0);
					if (i>0 && mlpLane.allowLeftTurn())
						((MLPLink)l).addLaneGraphEdge(mlpLane, (MLPLane) mlpLane.getLeftLane(),1.0);
					if (!((MLPSegment) seg).isEndSeg()){
						//可叠加封路判断
						for (int j = 0; j < mlpLane.nDnLanes(); j++) {
							MLPLane dnLane = (MLPLane) mlpLane.dnLane(j);
							((MLPLink)l).addLaneGraphEdge(mlpLane, dnLane, 0.0);
						}
					}
				}
			}
	...
	protected synchronized boolean addEdgeInRNGrapth(MLPLink l){
		if (l.getUpNode().equals(l.getDnNode())){
			System.out.println("loops in network: NodeID " + l.getUpNode().getId() + " LinkID " + l.getId());
			return false;
		}
...
//				System.out.println("DEBUG " + emitVeh.time + " " + emitVeh.laneIdx + " " + emitVeh.realVID);
				...
				//newVeh.loadSimulationFiles(getNewVehID(), 1, MLPParameter.VEHICLE_LENGTH, (float) validDis, (float) now);
...
//			double[][] servedRecords = Arrays.copyOf(servedRecordsObj,servedRecordsObj.length,double[][].class);
	...
	public void resetNetwork(long seed) {
		...
		for (int i = 0; i < nNodes(); i++) {
			mlpNode(i).reset();//从Node上清除未加入路段的车辆与运行参数
		}
...
//			GraphPath<Node, Link> gpath = DijkstraShortestPath.findPathBetween(this, oriNode, desNode);
			...
			ODPair newPair = Installer.getInstance(ODPair.class).initOD(oriNode, desNode);
	...
	public MLPVehicle generateVeh() {
		...
		if (!vehPool.isEmpty())
			...
			newVeh = Installer.getInstance(MLPVehicle.class);
			newVeh.setMlpParameter((MLPParameter) simParameter);
		}
		newVeh.updateUsage();
		veh_list.add(newVeh);
		return newVeh;
	...
	public void loadInflowFromSQL(String tableName, double fTime, double tTime){
		String sql = "SELECT * FROM public." + tableName +
				...
				" AND emitTime < " + tTime +
				" ORDER BY rvid";
		List<Object[]> results = DBUtils.queryDB(sql,"singleEmitDbName");
		Long theLNID = Long.MIN_VALUE;
		MLPLink theLNK = null;
	...
	public void loadInflowFromTripPathRec(LocalDateTime fromTime, LocalDateTime toTime, String nodeList, boolean fromDb,String sourceName){
		try {
			ArrayList<TripPathRecord> records;
			if (fromDb)
				records = TripPathProcess.QueryTripPath(fromTime, toTime,sourceName);
			else {
				if (bReader == null) {
				...
				while ((readLine = bReader.readLine()) != null){
					double emitTime = getSimClock().parseTime(readLine.split(",")[2]);
					if (emitTime >= getSimClock().secondsUntil(fromTime)
							&& emitTime <= getSimClock().secondsUntil(toTime)){
...
//			ArrayList<TripPathRecord> fixedFTime = TripPathProcess.estimateViaTime(trimRecords);
	...
	public void loadInflowFromLinkInOutRec(LocalDateTime fromTime, LocalDateTime toTime, String nodeList, boolean fromDb,String sourceName){
		try {
			List<LinkInOutRecord> records;
			if (fromDb) {
				records = LinkIOProcess.getLinkIORecords(fromTime, toTime, nodeList, sourceName);
				LinkIOProcess.append2InFlow(records,this,getSimClock().getCurrentLocalDateTime());
			}
			else {
				System.out.println("未实现功能");
			}
		}
		catch (Exception e){
			e.printStackTrace();
		...
		for (int i = 0; i < veh_list.size(); i++) {
			...
			if (!dealedVehID.contains(veh.getId())){
				...
				if (bblocking!=null && bblocking.equals(veh)) {
					if (getSysRand().nextDouble()<=1.0){//reduce chance to process
						if (true){//blocking.getLength()==veh.getLength()
							//switch lane
