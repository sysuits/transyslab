...
public class MLPEngine extends SimulationEngine{
	...
	//引用路网结构
	protected MLPNetwork mlpNetwork;

	//引擎运行输入文件配置
	protected HashMap<String,String> runProperties;
	//引擎运行参数
	private int repeatTimes;

	protected String rootDir;
	protected String emitSource;
	...
	public MLPEngine(){
		...
		mlpNetwork = Installer.getInstance(MLPNetwork.class);
		mlpNetwork.setEngine(this);
		...
		Installer.overrideModule(new AbstractModule() {
			...
			protected void configure() {
				bind(VehicleData.class).to(MLPVehData.class);
			}
		});
	}
	...
	public void config(String masterFilePath) {
		String tmp = new File(masterFilePath).getParent();
		rootDir = (tmp==null||tmp.equals("")) ? "" : tmp + "/";
	...
	public int simulationLoop() {
		...
		if (now >= updateTime_){
			...
			if (broadcasting()){
				...
				String msg = Thread.currentThread().getName() + " info: time " + getSimClock().getCurrentLocalDateTime().format(SimulationClock.DATETIME_FORMAT)
						+ " agent counts: " + num
		...
		if (now >= loadTime) {
			...
			if (needRndETable) {
				...
				if (sourceType.equals("SQL"))
					...
					String sourceName =  runProperties.get("emitSource");
					boolean fromDb = sourceType.contains("MPP");
					LocalDateTime fromTime = getSimClock().format(loadTime);
					LocalDateTime toTime = getSimClock().format(loadTimeEnd);
					mlpNetwork.loadInflowFromTripPathRec(fromTime,toTime,config.getString("nodes2Construct"),fromDb,sourceName);
				}
				else if (sourceType.contains("LinkInOut")) {
					String sourceName =  runProperties.get("emitSource");
					boolean fromDb = sourceType.contains("HBASE");
					LocalDateTime fromTime = getSimClock().format(loadTime);
					LocalDateTime toTime = getSimClock().format(loadTimeEnd);
					mlpNetwork.loadInflowFromLinkInOutRec(fromTime,toTime,config.getString("nodes2Construct"),fromDb,sourceName);
				}
			}
...
//				//for mlc only
//				for (int i = 0; i < mlpNetwork.nLinks(); i++) {
//					mlpNetwork.mlpLink(i).menditoryLaneChange();
//				}
//			}
			//车辆速度计算
			for (int i = 0; i < mlpNetwork.nLinks(); i++){
...
//				mlpNetwork.writeStat(statFileOut);
	...
	protected int loadSimulationFiles(){
...
//			mlpNetwork.calcStaticInfo();
		...
		if (!(config.getString("sensorPath") == null || config.getString("sensorPath").equals("")))
			readSensors(runProperties.get("sensorPath"));
	...
	public void readSensors(String path) {
		XmlParser.parseSensors(mlpNetwork, path);
	}
	...
	public void readSignalPlan2(String fileName) {
		...
		try {
			...
			for (int i = 1; i < results.size(); i++) {
				...
				double stime = getSimClock().parseTime(results.get(i).get("FTIME").substring(0,21));
				double etime = getSimClock().parseTime(results.get(i).get("TTIME").substring(0,21));
...
//			System.out.println("warning: have no empirical microscopic data read in");
	...
	protected void initTrackWriter(String threadName){
	...
	public void writeTrack(){
		if (!mlpNetwork.veh_list.isEmpty() ) {//&& now - 2*Math.floor(now/2) < 0.001
			...
			else if (config.getString("outputType").equals("vehicledataold")){
				int LV;
				int FV;
				for (MLPVehicle v : mlpNetwork.veh_list) {
					LV = 0; FV = 0;
					...
					if (v.trailing != null)
						FV = v.trailing.getId();
					trackWriter.write(
							getSimClock().format(now).format(SimulationClock.DATETIME_OUT) + "," +
									v.rvId + "," +
									v.getId() + "," +
									v.virtualType + "," +
									v.buffer + "," +
									v.getSegBasedLocationRef() + "," +
									v.Displacement() + "," +
									v.getCurrentSpeed() + "," +
									LV + "," +
									FV + "," +
    ...
    public int repeatRun() {
		...
		for(int i = 0; i < Math.max(repeatTimes,1); i++){
			run();
			if (getState()==Constants.STATE_QUIT)
				return state_;
	...
	public void readInTrafficParameters(){
	...
	public void setFDParas(double vf_sd, double kj, double alpha, double beta, double km_cf){
		double simStep = getSimClock().getStepSize();

		double km_sd = kj / Math.pow(1+alpha*beta,1/alpha);
		double qm = km_cf>=km_sd ?
				qmSD(kj,vf_sd,alpha,beta) :
				SD(km_cf,vf_sd,kj,alpha,beta)*km_cf;
		double vf_cf = qm/km_cf;

		double xc = 1/km_cf;
		double ts = vf_cf/(xc-1/kj) - simStep;

		setParameter("qm",qm);
		setParameter("vf_cf",vf_cf);
		setParameter("vf_sd",vf_sd);
		setParameter("kj",kj);
		setParameter("ts",ts);
		setParameter("xc",xc);
		setParameter("alpha", alpha);
	...
	public double qmSD(double kj, double vf, double alpha, double beta){
		double r = alpha*beta;
		double top = kj*vf*Math.pow(r/(1+r),beta);
		double bottom = Math.pow(1+r,1./alpha);
		return top/bottom;
	}

	public double SD(double k, double vf, double kj, double alpha, double beta){
		k = Math.min(k,kj);
		return vf * Math.pow(Math.pow(1-(k/kj),alpha),beta);
	}
