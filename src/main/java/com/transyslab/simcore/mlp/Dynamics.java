...
//			}
	...
	public double cfFun(MLPVehicle theVeh){
		...
		if (gap <= 0){
			((MLPNetwork)link.getNetwork()).broadcast("DEBUG: car crush " + theVeh.getId() + " → " +theVeh.leading.getId() + " at " + theVeh.getLink().getNetwork().getSimClock().getCurrentLocalDateTime().toString());
			return 0;
		}
		double vlead = theVeh.leading.getCurrentSpeed();
		return cfFun(gap,vlead);
	}
	public double cfFun(double gap, double vlead){
...
//				System.out.println("BUG CF函数输出异常");
	...
	public double updateHeadSpd(MLPVehicle headVeh){
		MLPLane nextLane = headVeh.nextLane();
		if (nextLane==null)
			nextLane = headVeh.lane.connectedDnLane;
		if (nextLane!=null&&!nextLane.equals(headVeh.lane.connectedDnLane)){
			//other passage channel (joint lane)
			if (headVeh.getDistance()<mlpParameter.alertDis()){
				if (nextLane.checkVolum(headVeh)){
					return nextLane.maxTransposeSpd(headVeh.getDistance());
				}
				double gap = Math.max(0.0, nextLane.rearBumperDsp()-headVeh.Displacement());
				double vlead = nextLane.getTail().getCurrentSpeed();
				return cfFun(gap,vlead);
			}
			return freeRun(headVeh);
		}
		...
		if (headVeh.leading != null && headVeh.getLane().vehsOnLn.contains(headVeh.leading)) {
	...
	public double freeRun(MLPVehicle headVeh){
		if (((MLPSegment)headVeh.link.getEndSegment()).endDSP - headVeh.Displacement() >
