...
//	private double releaseTime_;
	...
	private Map<Long, MLPLink> turnablePreviousLinks;
...
//		capacity = MLPParameter.getInstance().capacity;
		...
		turnablePreviousLinks = new HashMap<>();
	}
...
//		return r;
	...
	public void menditoryLaneChange(){
...
//						pr[i] = veh.calLCProbability2(i);
	...
	public void LCOperate(MLPVehicle veh, int turn) {
		...
		newVeh.selectedConn = veh.selectedConn;
...
//		NormalDistribution nd = new NormalDistribution(mean, sd);
	...
	public GraphPath<MLPLane, DefaultWeightedEdge> findLCRoute(MLPLane fLane, MLPLane tLane) {
		if (laneGraph.vertexSet().contains(fLane)&&laneGraph.vertexSet().contains(tLane))
			...
			((MLPLane)lane).dnStrmConns.forEach(conn -> {
				if (turnableNextLinks.get(conn.dnLinkID())==null){
					...
					if (((MLPNode) getDnNode()).dnLinkExist(tdLink)){
						...
						tdLink.turnablePreviousLinks.put(getId(),this);
					}
					else
	...
	public List<MLPLink> getTurnableUpLinks(){
		return new ArrayList<>(turnablePreviousLinks.values());
	}
