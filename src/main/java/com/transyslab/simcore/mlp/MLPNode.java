...
public class MLPNode extends Node{
	...
	public HashMap<String, Double> vMap;
	...
	public int serve(MLPVehicle veh) {
		...
		if((!ExpSwitch.CAP_CTRL) || lane_.checkPass()){
			...
			// at least one topology available down lane
			MLPLane nextLane = conns.get(0).dnLane;
			if (nVehsOn(nextLane)==0 && nextLane.checkVolum(veh.getLength(),0.0)) {
				//no priority control for now
				boolean canpass = true;
				for (int j = 0; j < nextLane.upStrmConns().size() && canpass; j++) {
					MLPLane confLane = nextLane.upStrmConns().get(j).upLane;
...
//		}
	...
	private void stashVeh(MLPVehicle veh, MLPLane nextLane, MLPConnector conn) {
		...
		if (conn!=null) {//交叉口运动
			...
			if (veh.newDis + nextLane.getLength() < 0.0) {
				...
				veh.newSpeed = veh.getDistance() / simClock.getStepSize();
	...
	protected void dispatchStatedVeh() {
		...
		for (int i = 0; i < statedVehs.size(); i++) {
			...
			if (veh.newDis<0.0 || (veh.newDis<1e-9 && type(Constants.NODE_TYPE_INTERSECTION)==0)) {
				if (!veh.lane.checkVolum(veh)) {
					veh.holdAtDnEnd();
					continue;
				}
				...
				double maxTranSpd = veh.lane.maxTransposeSpd(veh.getDistance());
				if (veh.newSpeed > maxTranSpd){
					veh.newSpeed = maxTranSpd;
					veh.newDis = veh.lane.minTransposeDis(maxTranSpd);
					timeAtPoint = now - (veh.lane.getLength()-veh.newDis)/maxTranSpd;
				}
...
//		}
	...
	public List<MLPLane> findTurningLanes(long fLinkID, long tlinkID){
		List<MLPConnector> conns = findTurningConn(fLinkID, tlinkID);
		if (conns!=null && conns.size()>0)
			return conns.stream().map(c->c.upLane).collect(Collectors.toList());
		else
			return null;
	}

	public int[] findTurningLaneIdx(long fLinkID, long tlinkID){
		List<MLPConnector> conns = findTurningConn(fLinkID, tlinkID);
		if (conns!=null && conns.size()>0)
			return conns.stream().mapToInt(c->c.upLane.getLnPosNum()).toArray();
		else
			return null;
	}
	...
	public void calNodeCapacity(){
		LinkedList<GeoPoint> points = new LinkedList<>();
		turningMap.forEach((dir,conns)->{
			conns.forEach(c ->{
				points.addAll(c.getShapePoints());
			});
		});
		if (points.size()>=2){
			points.sort(Comparator.comparing(GeoPoint::getLocationX));
			double dx = points.getLast().getLocationX()-points.getFirst().getLocationX();
			points.sort(Comparator.comparing(GeoPoint::getLocationY));
			double dy = points.getLast().getLocationY()-points.getFirst().getLocationY();
			double s = dx * dy;
			capacity = Math.min(s / ((MLPLink)getUpLink(0)).dynaFun.linkCharacteristics[2] / 3.6 * 0.2, 20);
		}
		else capacity = 1;
	}
	...
	public int nVehs(){
		return statedVehs.size();
	}

	public long nVehsOn(MLPLane dnLane){
		return statedVehs.stream().filter(v->v.lane.equals(dnLane)).count();
	}
}
