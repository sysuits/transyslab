...
public class TripPathProcess {
    ...
    public static ArrayList<TripPathRecord> QueryTripPath(LocalDateTime fromTime, LocalDateTime toTime, String mppName){
        return getInstance().query(fromTime, toTime, mppName);
    }

    protected ArrayList<TripPathRecord> query(LocalDateTime fromTime, LocalDateTime toTime, String mppName){
        ...
        String queryString =
                " select hphm,hpzl,ftime,route,sj_series from " +
                        "ana_rt_yw_trip_path"  +
                        " where ftime between '" + fTime + "'" +
                        " and '" + tTime + "'" +
                        " order by ftime";
        try {
            List<Object[]> rows = DBUtils.queryDB(queryString, mppName);
            rows.forEach(r->
                    result.add(
                            new TripPathRecord(
                                    (String)r[0],
...
//        }
    ...
    public static List ratioSampling(List sampleObjects,List<Double> probability, int sampleSize){
        ...
        for(int i=0;i<sampleSize;i++){
            // [0-1) 随机数
            double rnd = MLPNetwork.getSysRand().nextDouble();
