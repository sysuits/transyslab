...
public class EngThread extends Thread implements TaskWorker{
	...
	@Override
	public void worksWith(Task task) {
		long t_start = System.currentTimeMillis();

		controller.simulate(task);
		...
		if (logOn) {
			...
			if(broadcastNeeded) {
				...
				System.out.println("parameter: " + Arrays.toString(task.getInputVariables()) + "fitness: " + Arrays.toString(task.getObjectiveValues()));
			}
			writer.writeNFlush(Arrays.toString(task.getInputVariables())
					.replace(" ","")
					.replace("[","")
					.replace("]","") + "," +
					Arrays.toString(task.getObjectiveValues()).replace(" ","")
							.replace("[","")
							.replace("]","") + "," +
