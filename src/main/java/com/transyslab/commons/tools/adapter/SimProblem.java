...
public abstract class SimProblem extends AbstractDoubleProblem implements TaskGiver {
    protected String masterFileName;
    ...
    public void initWithoutEngThread(String masterFileName){
        this.masterFileName = masterFileName;
        this.config = ConfigUtils.createConfig(masterFileName);
        Installer.overrideModule(getModule());
    }

    public void initProblem(String masterFileName){
        initWithoutEngThread(masterFileName);
    ...
    protected EngThread createEngThread(){
        return getInjector().getInstance(EngThread.class);
    }
    ...
    public SimSolution validate(String solutionStr){
        SimSolution solution = (SimSolution) createSolution();
        String[] vars = solutionStr.split(",");
        for (int i = 0; i < vars.length; i++) {
            solution.setVariableValue(i,Double.parseDouble(vars[i]));
        }
        SimSolution ans = validate(solution);
        return ans;
    }

    public SimSolution validate(SimSolution solution){
        SimController controller = getInjector().getInstance(SimController.class);
        controller.config(masterFileName);
        controller.loadSimulationFiles();
        controller.simulate(solution);
        return solution;
    }
}
